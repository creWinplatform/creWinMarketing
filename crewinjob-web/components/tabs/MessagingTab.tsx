...
const SUB_TABS: { id: SubTab; icon: string; label: string; color: string; group?: string }[] = [
  { id: 'whatsapp',         icon: '📲', label: 'Click-to-WhatsApp',  color: 'bg-green-500',   group: 'Yeni Kullanıcı (İçerik)' },
  { id: 'telegram_channel', icon: '📢', label: 'Telegram Kanal',     color: 'bg-sky-500',     group: 'Yeni Kullanıcı (İçerik)' },
  { id: 'telegram_bot',     icon: '🤖', label: 'Telegram Bot',       color: 'bg-indigo-500',  group: 'Yeni Kullanıcı (İçerik)' },
  { id: 'notify_wa',        icon: '📩', label: 'WA Bildirim Gönder', color: 'bg-emerald-600', group: 'Mevcut Kullanıcı (Gönder)' },
  { id: 'notify_tg',        icon: '✈️', label: 'TG Kanal Gönder',   color: 'bg-cyan-600',    group: 'Mevcut Kullanıcı (Gönder)' },
];
...
export default function MessagingTab() {
  ...
  const [msgNumbers,   setMsgNumbers]   = useState('');     // satır satır numaralar (WA)
  const [msgChatIds,   setMsgChatIds]   = useState('');     // satır satır chat_id (TG)
  ...
  const sendMessage = async () => {
    ...
    try {
      if (subTab === 'notify_wa') {
        ...
        if (!waData.error) logActivity('wa_sent', `WhatsApp mesajı gönderildi (${msgMode})`, `${msgText.slice(0, 60)}…`);
      } else {
        ...
        if (!tgData.error) logActivity('tg_sent', `Telegram mesajı gönderildi (${msgMode})`, `${msgText.slice(0, 60)}…`);
      }
    } catch (e: unknown) {
      setSendResult({ error: e instanceof Error ? e.message : 'Hata oluştu' });
    } finally {
  ...
  const generateLabel = () => {
    ...
    if (subTab === 'whatsapp')         return '📲 WhatsApp Kampanyası Üret';
    if (subTab === 'telegram_channel') return '📢 Telegram İçeriği Üret';
    return '🤖 Telegram Bot Tasarımı Üret';
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1">
          {(['Yeni Kullanıcı (İçerik)', 'Mevcut Kullanıcı (Gönder)'] as const).map(group => (
            <div key={group}>
              ...
              {SUB_TABS.filter(t => t.group === group).map(t => (
                <button
                  key={t.id}
          ...
          <>
            ...
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg p-3 text-xs text-green-800 dark:text-green-300 flex gap-2">
              ...
              <div>
                ...
                <p className="leading-relaxed">Facebook/Instagram reklamında "WhatsApp'ta Yaz" butonu → kullanıcı kendi isteğiyle başlatıyor → bot karşılıyor → crewinjob.com'a kayıt.</p>
              </div>
            </div>
            ...
            <Field label="WhatsApp Numarası">
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500">Bot karşılama mesajlarında ve reklamlarda kullanılır.</p>
            </Field>

            <Field label="Özel Hedef (opsiyonel)">
              <textarea value={waCustomGoal} onChange={e => setWaCustomGoal(e.target.value)}
                placeholder="Örn: 500 yeni Filipino seafarer kaydı, CPL < $1"
                rows={2} className={`${selectClass} resize-none`} />
            </Field>
          </>
        )}

        {/* ── Telegram Kanal Formu ── */}
        {subTab === 'telegram_channel' && (
          <>
            <div className="bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-700 rounded-lg p-3 text-xs text-sky-800 dark:text-sky-300 flex gap-2">
              <span className="text-base shrink-0">📢</span>
              <div>
                <p className="font-semibold mb-1">Telegram Kanal Stratejisi</p>
                <p className="leading-relaxed">Günlük ilan akışı, sponsorlu postlar ve 0→10K abone büyüme planı. Tamamen ücretsiz broadcast.</p>
              </div>
            </div>
            ...
            <Field label="Kanal Adresi (opsiyonel)">
        ...
        {subTab === 'telegram_bot' && (
          <>
            <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-700 rounded-lg p-3 text-xs text-indigo-800 dark:text-indigo-300 flex gap-2">
              <span className="text-base shrink-0">🤖</span>
              <div>
                <p className="font-semibold mb-1">Telegram Bot Tasarımı</p>
                <p className="leading-relaxed">Kullanıcı botu açar → rütbe/gemi tipi soruları → kişiselleştirilmiş ilan özeti → crewinjob.com kayıt linki. Tam diyalog scripti + teknik dokümantasyon.</p>
              </div>
            </div>

            <Field label="Bot Tipi">
            ...
            <Field label="Ek Özellikler">
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500">Seçilenler bot tasarımına dahil edilir.</p>
            </Field>
          </>
        )}
        ...
        {subTab === 'notify_wa' && (
          <>
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 rounded-lg p-3 text-xs text-emerald-800 dark:text-emerald-300 flex gap-2">
              <span className="text-base shrink-0">📩</span>
              <div>
                <p className="font-semibold mb-1">WhatsApp Bildirim Gönder</p>
                <p className="leading-relaxed">Mevcut kullanıcılara doğrudan WhatsApp mesajı. Önce <code>WHATSAPP_PHONE_NUMBER_ID</code> ve <code>WHATSAPP_ACCESS_TOKEN</code> ayarlayın.</p>
              </div>
            </div>
            ...
            {msgConfig && !msgConfig.whatsapp.configured && (
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg p-3 text-xs text-red-700 dark:text-red-300">
                ⚠️ <strong>.env.local</strong> içinde <code>WHATSAPP_PHONE_NUMBER_ID</code> ve <code>WHATSAPP_ACCESS_TOKEN</code> tanımlanmamış.
              </div>
            )}

            <Field label="Gönderim Modu">
              <div className="grid grid-cols-2 gap-1.5">
                {([['single','Tekli'], ['broadcast','Toplu']] as const).map(([v, l]) => (
            ...
            <Field label={msgMode === 'broadcast' ? 'Telefon Numaraları (her satıra bir tane)' : 'Telefon Numarası'}>
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500">E.164 format — ülke kodu dahil (+90...)</p>
            </Field>

            <Field label="Mesaj">
              <textarea value={msgText} onChange={e => setMsgText(e.target.value)}
                placeholder="Merhaba! crewinjob.com'da yeni ilanlar sizi bekliyor. Hemen bakın: crewinjob.com"
                rows={4} className={`${selectClass} resize-none`} />
              <p className="text-[10px] text-slate-400 dark:text-slate-500">{msgText.length} karakter — WhatsApp limiti 4096</p>
            </Field>
          </>
        )}

        {/* ── Telegram Kanal Gönder ── */}
        {subTab === 'notify_tg' && (
          <>
            <div className="bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-700 rounded-lg p-3 text-xs text-cyan-800 dark:text-cyan-300 flex gap-2">
              ...
              <div>
                <p className="font-semibold mb-1">Telegram Gönder</p>
                <p className="leading-relaxed">Kanala, gruba veya tek kullanıcıya mesaj. <code>TELEGRAM_BOT_TOKEN</code> ve <code>TELEGRAM_CHANNEL_ID</code> gerekli.</p>
              </div>
            </div>

            {msgConfig && !msgConfig.telegram.configured && (
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg p-3 text-xs text-red-700 dark:text-red-300">
                ⚠️ <strong>.env.local</strong> içinde <code>TELEGRAM_BOT_TOKEN</code> tanımlanmamış.
              </div>
            )}

            <Field label="Gönderim Modu">
              <div className="grid grid-cols-3 gap-1.5">
                {([['channel','Kanal'], ['single','Tekli'], ['broadcast','Toplu']] as const).map(([v, l]) => (
                  <button key={v} onClick={() => setMsgMode(v)}
                    className={`text-xs py-2 rounded-lg font-medium transition-colors ${
              ...
              {msgMode === 'channel' && (
                <p className="text-[10px] text-slate-400 dark:text-slate-500">TELEGRAM_CHANNEL_ID env değeri kullanılır.</p>
              )}
            </Field>

            {(msgMode === 'single' || msgMode === 'broadcast') && (
              <Field label={msgMode === 'broadcast' ? 'Chat ID\'ler (her satıra bir tane)' : 'Chat ID veya @handle'}>
            ...
            <Field label="Mesaj (HTML destekler)">
              <textarea value={msgText} onChange={e => setMsgText(e.target.value)}
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500">Etiketler: &lt;b&gt;, &lt;i&gt;, &lt;a href&gt;, &lt;code&gt;</p>
            </Field>
          </>
        )}

        {/* Dil — sadece içerik üretimi sekmeleri için */}
        {!['notify_wa', 'notify_tg'].includes(subTab) && (
          <Field label="Çıktı Dili">
        ...
        {['notify_wa', 'notify_tg'].includes(subTab) && (
          <button onClick={sendMessage} disabled={sendLoading || !msgText.trim()}
            ...
            {subTab === 'notify_wa' ? '📩 WhatsApp Gönder' : '✈️ Telegram Gönder'}
          </button>
        )}
      </div>

      {/* Sağ panel — output veya gönderim sonucu */}
      <div className="lg:col-span-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
        {/* İçerik üretimi paneli */}
        {!['notify_wa', 'notify_tg'].includes(subTab) && (
          <>
            <h3 className="font-medium text-slate-700 dark:text-slate-200 text-sm mb-4 flex items-center gap-2">
              {subTab === 'whatsapp'         && <><span>📲</span> Click-to-WhatsApp Kampanya Paketi</>}
              {subTab === 'telegram_channel' && <><span>📢</span> Telegram Kanal İçeriği</>}
              {subTab === 'telegram_bot'     && <><span>🤖</span> Telegram Bot Tasarım Dokümanı</>}
            </h3>
            <OutputPanel
              content={output}
              loading={loading}
              ...
              autoSaveHistory={{ prompt: `Mesajlaşma · ${subTab}`, language }}
            />
          </>
        )}

        {/* Gönderim sonucu paneli */}
        {['notify_wa', 'notify_tg'].includes(subTab) && (
          <div className="flex flex-col gap-4">
            <h3 className="font-medium text-slate-700 dark:text-slate-200 text-sm flex items-center gap-2">
              {subTab === 'notify_wa' ? <><span>📩</span> WhatsApp Gönderim Durumu</> : <><span>✈️</span> Telegram Gönderim Durumu</>}
            </h3>
            ...
            <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-5 text-sm text-slate-600 dark:text-slate-300 flex flex-col gap-3">
              <p className="font-semibold text-slate-700 dark:text-slate-200">Nasıl çalışır?</p>
              {subTab === 'notify_wa' ? (
              ...
              <div className={`rounded-xl p-5 border ${
                ...
                {sendResult.error ? (
                  <div className="flex gap-2 items-start text-red-700 dark:text-red-300">
                    ...
                    <div>
                      <p className="font-semibold text-sm">Gönderim başarısız</p>
                  ...
                  <div className="flex gap-2 items-start text-green-700 dark:text-green-300">
                    <span className="text-lg">✅</span>
                    <div>
                      <p className="font-semibold text-sm">Gönderim tamamlandı</p>
                      {sendResult.sent !== undefined && (
                        <div className="mt-2 flex gap-4 text-xs">
                          <span>✓ Gönderilen: <strong>{sendResult.sent}</strong></span>
                          {(sendResult.failed ?? 0) > 0 && (
                            <span className="text-amber-600 dark:text-amber-400">✗ Başarısız: <strong>{sendResult.failed}</strong></span>
                          )}
                        </div>
                      )}
                      {sendResult.success && sendResult.sent === undefined && (
                        <p className="text-xs mt-1">Mesaj başarıyla iletildi.</p>
                      )}
                    </div>
                  </div>
                )}
            ...
            {!sendResult && !sendLoading && (
              <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-300 dark:text-slate-600">
                ...
                <p className="text-xs text-center">Mesajı doldurup gönder butonuna tıklayın</p>
              </div>
            )}
          </div>
        )}
